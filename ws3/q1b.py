...
def int_f(a,b): return ( sin(b) - b*cos(b) - sin(a) + a*cos(a) )
...
hs = log10(hs)
midp_errs = log10(midp_errs)
trap_errs = log10(trap_errs)
simp_errs = log10(simp_errs) 
plt.figure()
fs = 20;
plt.plot(hs,midp_errs,label="Midpoint")
plt.plot(hs,trap_errs,label="Trapezoid")
plt.plot(hs,simp_errs,label="Simpson")
plt.title("f(x) = x*sin(x)",fontsize=fs)
