...
def main(pc,linstyle):
    ...
    def hlle(hyd):
        ...
        for i in range(1,hyd.n-2):
            ...
            fluxr[0,i] = hyd.qm[0,i]*hyd.velm[i]
    ...
    while(t < tend):
        ...
        #if i==0:
        #    mpl.subplot(221)
        #    mpl.plot(hyd.x,hyd.rho,"r-",label="t=%.2f" % t)
        #    mpl.legend()
        #    mpl.ylabel(r"$\rho$")
        #elif i==250:
        #    mpl.subplot(222)
    ...
    mpl.plot(hyd.x,hyd.rho,linstyle,label=pc)
    mpl.legend()
    mpl.xlabel(r"$r$")
...
mpl.xlabel(r"$r$")
mpl.ylabel(r"$\rho$")
#mpl.savefig("type_comparison.png")
mpl.show()
